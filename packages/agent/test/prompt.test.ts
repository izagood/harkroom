...
describe('buildTurnPrompt — 팀 호출(047)', () => {
  ...
  /**
   * **넘기는 방법을 말한다**(2026-09-15 실측으로 고친 자리).
   *
   * 실제 팀 호출을 분석해 보니 팀장이 `message.delegate` 를 **한 번도 쓰지 않았다** —
   * 평범한 멘션으로 불렀다. 도구는 있는데 프롬프트가 옛 지시(*"`@handle` 로 부르고 본문
   * 맨 앞에 둬라"*)를 그대로 갖고 있었기 때문이다. 그래서 의무도 기한도 결말도 만들어지지
   * 않았고, 3단계 전체가 **한 번도 돌지 않았다.**
   */
  it('넘길 때는 message.delegate 를 쓰라고 말한다', () => {
    const { prompt } = call();
    expect(prompt).toContain('message.delegate');
    // 평범한 멘션은 서버가 위임으로 알지 못한다 — 그 이유까지 적는다.
    expect(prompt).toContain('결말이 너에게 돌아오지 않는다');
  });

  it('기다리지 말되 다시 깨어난다고 말한다 — 옛 "기다리지 마라"는 이제 거짓이다', () => {
    const { prompt } = call();
    // 3-1 이 왕복을 열기 전의 문장("넘긴 답을 기다리지는 마라")이 남아 있으면 팀장은
    // 결말이 돌아온다는 것을 모르고 turn.wake 로 스스로 예약하거나 그냥 포기한다.
    expect(prompt).not.toContain('기다리지는 마라');
    expect(prompt).toContain('다시 깨어난다');
    // 결말 네 갈래를 미리 알려 준다 — 깨어난 뒤에 할 일을 그때 배우지 않게.
    expect(prompt).toContain('무응답');
  });

  /**
   * **넘긴 뒤에도 자기 몫은 계속한다**(2026-09-16 실측).
   *
   * 앞 판의 문장(*"넘기고 턴을 끝내라"*)은 그것을 금지하는 것처럼 읽혔는데, 실측에서 팀장이
   * 넘긴 뒤 자기 조사를 이어 가 **데모 전에 알아야 할 것**을 먼저 올렸고 그 판단은 유용했다.
   * 규칙이 너무 좁았던 것이라 금지를 풀고 경계만 남긴다.
   */
  it('넘긴 뒤에도 네 몫은 계속하되 최종 답은 결말 뒤라고 말한다', () => {
    const { prompt } = call();
    expect(prompt).toContain('네가 할 수 있는 일은 계속해라');
    expect(prompt).toContain('최종 답은 결말이 난 뒤');
    // 경계는 하나다 — 넘긴 일의 결과를 짐작해 미리 결론 내지 않는 것.
    expect(prompt).toContain('짐작해서 미리 결론');
  });

  it('지시와 보고를 한 메시지에 섞지 말라고 말한다', () => {
    // 실측: 팀장의 위임 메시지가 팀원과 요청자를 **함께** 불렀다. 그러면 사람은 자기에게
    // 온 말과 팀 안의 말을 구별할 수 없고, 창구를 하나로 좁힌 뜻이 사라진다.
    expect(call().prompt).toContain('최종 답 하나');
  });
...
describe('buildTurnPrompt — 넘긴 일의 결말(050)', () => {
  ...
  /**
   * **목록만 읽고 다시 시키는 것**을 막는다(2026-09-16 실측).
   *
   * 팀원이 *"닫았습니다"* 라고 답한 뒤인데도 팀장이 같은 지시를 다시 보냈고, 팀원은
   * *"앞 메시지가 엇갈린 것 같다"* 고 답했다. 이 블록은 결말을 **목록**으로 주므로 그것만
   * 읽으면 무엇이 이미 됐는지 모른다 — `끝남` 은 "무엇을 했는지" 를 말하지 않는다.
   */
  it('팀원의 보고를 먼저 읽으라고 말한다 — 목록은 무엇을 했는지 말하지 않는다', () => {
    const { prompt } = call({
      timedOut: false, roundsLeft: 2, items: [{ handle: 'scout', outcome: 'done' }],
    });
    expect(prompt).toContain('먼저 읽어라');
    // 그 줄은 **목록 바로 뒤**에 선다 — 가장 흔한 실수를 가장 가까운 자리에서 막는다.
    expect(prompt.indexOf('@scout — 끝남')).toBeLessThan(prompt.indexOf('먼저 읽어라'));
  });
