...
describe('workspaceName', () => {
  it('같은 스레드라도 에이전트가 다르면 이름이 다르다 (spec §3 다중 에이전트)', () => {
    const a = workspaceName('acct-1', 'ch1/m9');
    const b = workspaceName('acct-2', 'ch1/m9');
    expect(a).not.toBe(b);
    expect(a).toMatch(/^harkroom-acct-1-[0-9a-f]{8}$/);
  });
  ...
  it('같은 handle·threadKey 는 몇 번을 불러도 같은 이름을 낸다', () => {
    expect(workspaceName('acct-1', 'ch1/m9')).toBe(workspaceName('acct-1', 'ch1/m9'));
  });
  ...
  it('이름이 바뀌어도 이미 있는 워크스페이스를 쓴다', () => {
    const before = workspaceName('acct-1', 'ch1/m9');
    expect(workspaceName('acct-1', 'ch1/m9', [before])).toBe(before);
  });

  /** 다른 스레드의 디렉터리를 주워 오지 않는다 — 고르는 기준은 **그 스레드의 해시**다. */
  it('다른 스레드의 워크스페이스는 물려받지 않는다', () => {
    const other = workspaceName('acct-1', 'ch1/m1');
    expect(workspaceName('acct-1', 'ch1/m9', [other])).toBe(workspaceName('acct-1', 'ch1/m9'));
  });

  /** 첫 턴(뿌리가 비었다)은 예전 그대로다. */
  it('물려받을 것이 없으면 새 이름으로 짓는다', () => {
    expect(workspaceName('acct-1', 'ch1/m9', [])).toBe('harkroom-acct-1-' + workspaceName('acct-1', 'ch1/m9').split('-').pop());
  });

  /**
   * **새로 짓는 이름에서 에이전트를 빼지 않는다**(#850 이후로 그 자리는 id 다). 이 이름은 디렉터리 이름이면서 동시에
   ...
   */
  it('물려받는 것은 이미 있을 때뿐이고, 새 이름은 여전히 에이전트로 갈린다', () => {
    expect(workspaceName('acct-1', 'ch1/m9', [])).not.toBe(workspaceName('acct-2', 'ch1/m9', []));
  });
});
...
describe('resolveWorkspaceName (#846)', () => {
  it('디스크에 이미 있는 이름을 물려준다', async () => {
    ...
    expect(await resolveWorkspaceName(base, 'acct-1', 'ch1/m9')).toBe(before);
  });

  /** 뿌리가 아직 없는 첫 턴에 던지지 않는다 — 그때는 새 이름이다. */
  it('뿌리가 없어도 새 이름을 낸다', async () => {
    const missing = join(tmpdir(), 'ws-none-', String(Date.now()));
    expect(await resolveWorkspaceName(missing, 'acct-1', 'ch1/m9')).toBe(workspaceName('acct-1', 'ch1/m9'));
  });
});
...
describe('ensureWorkspace', () => {
  it('디렉터리가 없으면 avcs workspace project 를 부른다', async () => {
    ...
    expect(dir).toContain('harkroom-acct-1-');
  });
  ...
  it('디렉터리가 이미 있으면 exec 를 아예 부르지 않고 그 경로를 바로 돌려준다', async () => {
    ...
    const name = workspaceName('acct-1', 'ch1/m9');
    ...
    const dir = await ensureWorkspace(exec, { agentId: 'acct-1', threadKey: 'ch1/m9', baseDir, repoDir: '/repo' });
  ...
  it('워크스페이스 경로에 디렉터리가 아닌 파일이 있으면 폴백하지 않고 던진다', async () => {
    ...
    await expect(
      ensureWorkspace(exec, { agentId: 'acct-1', threadKey: 'ch1/m9', baseDir, repoDir: '/repo' }),
  ...
  it('avcs repo 가 아니면 격리 없이 repoDir 로 폴백한다 — 채팅 전용 에이전트가 죽으면 안 된다', async () => {
    ...
    const dir = await ensureWorkspace(exec, { agentId: 'acct-1', threadKey: 'k', baseDir: '/tmp/x', repoDir: '/repo' });
  ...
  it('그 외 project 실패는 stderr 를 담아 던진다 — 조용한 실패 금지', async () => {
    ...
    await expect(
      ensureWorkspace(exec, { agentId: 'acct-1', threadKey: 'k', baseDir: '/tmp/x', repoDir: '/repo' }),
