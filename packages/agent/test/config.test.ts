...
describe('loadConfig — HARKROOM_AGENT_INSTANCE (#174)', () => {
   ...
   */
  it('빈 문자열은 없는 것으로 본다', () => {
    expect(loadConfig({ HARKROOM_PAT: 'murp_test', HARKROOM_AGENT_INSTANCE: '' }).agentInstance)
      .toBeUndefined();
  });

  it('소문자·숫자·하이픈 1~32자는 그대로 통과한다', () => {
    const env = { HARKROOM_PAT: 'murp_test' };
  ...
  it.each([
    ...
    expect(() => loadConfig({ HARKROOM_PAT: 'murp_test', HARKROOM_AGENT_INSTANCE: value }))
      .toThrow(/HARKROOM_AGENT_INSTANCE 가 유효하지 않다/);
  });

  /** 메시지에 **받은 값과 허용 문법**이 함께 있어야 운영자가 무엇을 고칠지 안다. */
  it('실패 메시지가 받은 값과 허용 문법을 함께 말한다', () => {
    expect(() => loadConfig({ HARKROOM_PAT: 'murp_test', HARKROOM_AGENT_INSTANCE: 'Bad' }))
