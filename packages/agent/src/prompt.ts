...
function teamSection(team: InboxTeamCall, meId: string, handles: Record<string, string>): string[] {
  ...
  return [
    ...
    '',
    '**기다리지 말고 턴을 끝내라 — 그래도 네가 다시 깨어난다.** 넘긴 일이 전부 끝나거나',
    '기한이 지나면 서버가 너를 깨우고, 그때 팀원마다 `끝남`·`실패`·`무응답`·`취소` 중 무엇인지',
    '받는다. 그 턴에서 취합해 최종 답을 쓰면 된다. 기한은 기본 10분이고 `deadlineSec` 로',
    '늘릴 수 있는데, 팀원이 진행을 올리거나 그 턴이 도는 동안은 자동으로 밀린다.',
    '',
