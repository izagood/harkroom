...
export class ApiClient {
  ...
  /**
   * 에이전트를 **명부에서 내린다**(#836). 되돌리는 길이 없다 — 비활성화(`setAgentDisabled`)와
   * 다른 조작이라 라우트도 다르다.
   *
   * 응답이 없다(204). 서버가 그 에이전트를 목록에서 빼므로 돌려줄 정의가 없다 — 부르는 쪽은
   * 목록에서 지우면 된다.
   */
  async deleteAgent(id: string): Promise<void> {
    return this.req('DELETE', `/accounts/agents/${id}`);
  }

  /**
   * 러너에게 **종료를 요청한다**(#129). 재시작이 아니다 — harkroom 는 러너를 띄우지 않으므로
