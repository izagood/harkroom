...
const agentView = (extra: Partial<AgentView> = {}): AgentView => ({
  id: AGENT_ID, handle: 'forge', displayName: 'forge', kind: 'agent', isAdmin: false, role: 'member', assignment: null, invokeScope: 'community', credentialScope: 'none', invokers: [], mcpServers: [],
...
/**
 * **복수형이 이 화면에서도 실제로 갈린다.** 뒤처진 러너의 개수가 곧 사람이 갱신해야 할
 * 오퍼레이터의 범위다 — `1 runners` 가 뜨면 그 문장이 급조된 것으로 읽히고 사람은 숫자를
 * 다시 확인하지 않는다. (재기동 버튼은 사라졌다 — 스펙 2026-09-20 §2.)
 */
describe('에이전트 설정 — 개수를 말하는 문장의 복수형', () => {
  ...
  it('한국어는 수에 따라 명사가 안 바뀐다', () => {
    const t = translator('ko');
    expect(t('agents.stale.here', { count: 1 })).toContain('러너 1대');
    expect(t('agents.stale.here', { count: 7 })).toContain('러너 7대');
...
describe('갤러리 — 설명은 사전을 지나고 견본은 안 지난다', () => {
   ...
   * 그것은 harkroom 가 하는 말이 아니라 예시로 박아 둔 남의 말이고, 이 화면이 가르치는
