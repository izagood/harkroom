...
| `HARKROOM_HARNESS_ADAPTERS` | Routes harness-specific facts through the adapter table (`packages/agent/src/adapters/`). **On by default** since the two paths were proven to answer identically (parity tests plus a CI run of the agent suite on each) and codex was verified to actually answer through the TUI path. Set `0` or `false` to fall back to the old per-harness branches on a whole runner; a single agent can be pinned back with `executionPath: 'legacy'`. The old branches and this variable are removed in a later release | on | No |
