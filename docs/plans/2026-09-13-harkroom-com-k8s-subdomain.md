...
| # | 무엇 |
|---|---|
| 1 | ~~private 레포 `harkroom-gate` 생성, grant/job 스키마~~ **완료** |
| 2 | ~~초대 코드 발급 CLI~~ **완료** + **관리 콘솔**(`/admin`, 기본 꺼짐·내부 전용) |
| 3 | ~~**harkroom 서버에 `POST /claim`** + 마이그레이션 053 + 레이트 리밋(4-2)~~ **완료** (#812, v0.1.223) |
| 4 | ~~`POST /api/workspaces` — 검증·선점·이메일 저장·토큰 생성·커밋까지~~ **완료** (SealedSecret 봉인 포함 — 6-3) |
...
### 6-3. 백업 복구와 SealedSecret 봉인을 실물로 확인했다 (2026-09-17~18)

**복구 리허설 — 백업이 실제로 복원된다.**

캐너리를 심고(DB 행 + 첨부 파일) 백업을 돌린 뒤, **별도 네임스페이스에 CNPG
`bootstrap.recovery` 로 되살려** 대조했다.

| | 결과 |
|---|---|
| CNPG 백업 → MinIO | base + WAL 이 `postgres-v1/` 에 |
| 첨부 미러 → MinIO | 파일 그대로 |
| **DB 복원** | 캐너리 행 + 마이그레이션 54개 그대로 |
| **첨부 복원** | 내용 일치 |

VolumeSnapshot 때는 복원에서 막혔다(1-1 각주). 이번에는 끝까지 갔으므로 **이 경로는
백업이라고 불러도 된다.**

배포 과정에서 둘이 걸렸고 고쳤다:
- `mc` 이미지가 Docker Hub 에서 **당겨지지 않는다**(MinIO 가 배포를 제한했다). `quay.io`
  로 옮겼다(#120). **rowlol 도 같은 이미지를 쓴다** — 지금은 캐시로 살아 있지만 파드가
  옮겨 가면 같은 실패를 만난다
- 미러 로그가 `mirrored 0 files` 라고 **거짓을 말했다**. 그 이미지에 `find` 가 없어서
  항상 0 이었고, 진짜 0 개일 때와 구별되지 않았다 — 조용한 실패를 감시하려던 줄이 그
  자체로 신호를 망가뜨리고 있었다(#121)

**SealedSecret 봉인 — 프로덕션 컨트롤러가 복호화했다.**

gate 가 `kubeseal` 바이너리 없이, **클러스터 자격증명 없이** 봉인한다. 근거: 컨트롤러가
공개 인증서를 인증 없이 HTTP 로 내준다(`sealed-secrets.sealed-secrets.svc:8080/v1/cert.pem`).
그 공개키만 있으면 봉인은 순수 계산이다.

시험이 아니라 **실물로 확인했다**: 클러스터 인증서로 봉인한 SealedSecret 을 실제
컨트롤러에 apply 하니 복호화됐고(`DECRYPTED: sealed-ok-...`), 같은 blob 의 네임스페이스만
바꾸면 `no key could decrypt secret` 으로 거절됐다 — **봉인이 ns 에 묶인다는 보증**이
실물로 선다.

> 설계 메모: 범위는 GCM 의 AAD 가 아니라 **RSA-OAEP 의 label** 에 들어간다
> (`<namespace>/<name>`). AAD 로 짜면 봉인은 성공하고 **배포 시점에야** 실패한다.
