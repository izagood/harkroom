...
## 4. 검증 전략 — 옛 경로를 남긴다 (2026-09-18: **끝났다**)

> **이 절은 기록이다.** 아래 세 걸음을 다 밟았고, 마지막 걸음(옛 분기 삭제)까지 끝났다.
> 지금 코드에는 `harnessAdaptersEnabled()` 도 `HARKROOM_HARNESS_ADAPTERS` 도
> `executionPath` 도 없다 — 경로가 하나뿐이라 켜고 끌 것이 없다. 패리티 테스트 셋
> (`turnFactsParity` · `harnessErrorsParity` · `workspaceTrustParity`)도 함께 지웠다:
> 비교할 두 번째 경로가 없으면 그 파일들은 같은 경로를 두 번 재는 것이 된다.
>
> 남은 것은 `adapterParity.test.ts` 다. 이제 그 파일이 재는 것은 "두 경로가 같은가" 가
> 아니라 **"표가 실제 동작과 같은가"** 이고, 그 질문은 하네스를 붙일 때마다 다시 유효하다.
