...
- 에이전트 참여: **외부 접속형** — 서버는 에이전트 런타임을 모르고, 에이전트는
  ...
  - #250: 그 '외부'가 **데스크탑 앱**이 될 수 있다. 앱은 내가 소유한(`ownerAccountId`)
    ...
    `HARKROOM_PAT` 로 넘긴다. **서버는 여전히 러너를 띄우지 않는다** — 결정이
  ...
  - 스펙 [2026-09-20](specs/2026-09-20-operator-and-permissions-design.md): 그 daemon 이
    **오퍼레이터**가 됐고, 띄우라고 말하는 쪽이 **앱에서 서버로** 옮겨 갔다. 앱은 러너를
    띄우지도 PAT 를 쥐지도 않는다 — 사람이 앱에서 하는 일은 오퍼레이터를 등록하고
    에이전트를 거기에 **배정**하는 것뿐이고, 오퍼레이터가 서버에 outbound WS 로 붙어 그
    배정을 받아 띄운다. 서버를 원격지로 옮기자 드러난 결함(앱이 없으면 에이전트도 없다)의
    답이다. 위 #250 문단의 "앱이 PAT 를 키체인에 보관해 넘긴다"는 그래서 **과거**다 — 단계
    2~3 동안은 오퍼레이터가 PAT 를 받아 넘기고, 단계 4 가 PAT 자체를 없앤다.
...
harkroom/
  ...
  packages/operator   # 러너 프로세스의 소유자 (기계마다 하나; 서버에 붙어 배정을 받고, 앱과는 unix 소켓)
  packages/desktop    # Tauri 2 + React
  packages/shared     # 프로토콜 타입·스키마 (server/desktop/agent/operator 공유)
```

MVP 제외: cli, 모바일, 웹 UI, 상주 에이전트 러너.

#### 오퍼레이터와 server 의 경계 — **누가 무엇의 주인인가**

`packages/operator`(옛 이름 daemon)은 뒤늦게 생긴 패키지이고, 그 공백 때문에 *"에이전트
관리는 daemon 이 하니 에이전트에 관한 통지도 daemon 이 중계하면 되지 않나"* 라는 물음이
반복됐다. 앞 판본의 답은 *"daemon 은 서버를 모르고 서버는 daemon 을 모른다"* 였고, 스펙
[2026-09-20](specs/2026-09-20-operator-and-permissions-design.md) §8 이 그 결정 **하나**를
뒤집었다 — 근거 셋에 각각 답하면서.

| 층 | 무엇의 주인인가 | 누구에게 말하나 |
|---|---|---|
| **오퍼레이터** | 이 기계의 **프로세스** — 떴다·죽었다·종료 코드·고아 입양, 그리고 이 기계의 **능력**(어느 에이전트를 돌릴 수 있나·하네스가 있나) | 서버 하나당 outbound WS 하나(`/operator`), 그 기계의 앱에는 unix 소켓 |
| **server** | **워크스페이스의 사실** — 누가 불렸나·그 요청이 처리됐나·러너가 붙어 있나, 그리고 **배정**(어느 에이전트를 어느 오퍼레이터가 돌리나) | 그것을 볼 수 있는 전원 |

| 앞 판본의 근거 | 지금의 답 |
|---|---|
| ① **닿는 범위** — unix 소켓은 그 기계 안이다 | 오퍼레이터가 서버에 **outbound WS** 로 붙는다. 러너가 릴레이로 하던 것과 같은 방향·같은 이유. 다른 기계·다른 사람의 앱도 서버를 통해 이 오퍼레이터에 닿는다 |
| ② **어휘** — daemon 이 서버 어휘를 가지면 진실이 두 곳에 산다 | 오퍼레이터의 어휘를 **셋으로 제한**한다: 능력·프로세스·배정 수신. "누가 불렸나·처리됐나"는 여전히 서버만 안다. 러너의 프레임은 오퍼레이터를 **지나갈 뿐** 오퍼레이터가 해석하지 않는다(단계 3) |
| ③ **커뮤니티 격리** — daemon 은 기계마다 하나라 커뮤니티마다가 아니다 | 여전히 기계마다 하나다. 격리는 스코핑 조건이 아니라 **인스턴스**로 강제한다(`community.ts`, 데스크탑 `communities.ts` 와 같은 패턴). 러너 상태는 이미 서버별 UUID 로 갈라져 있다 |

바뀌지 않은 것: **서버는 러너를 띄우지 않는다**(배정을 내려보낼 뿐이다), 오퍼레이터가
소유하는 것은 프로세스이지 세션이 아니다(`sessions.json` writer 는 러너 하나), 어느 쪽도
포트를 열지 않는다. 그리고 **앱은 컨트롤 플레인이 아니다** — 앱은 배정을 고르는 프론트이고,
그래서 어느 기기에서든(앱이 꺼져 있어도) 같은 에이전트가 같은 곳에서 돈다.
