...
오퍼레이터에 배정되지 않은 에이전트의 러너는 지금도 사람이 띄운다(단계 4 까지 — 그 뒤로는
러너가 서버에 직접 붙지 않는다). 앱의 "러너 실행" 명령 틀은 사라졌다(앱은 러너를 모른다).
두 갈래다:
...
**손으로 띄운 러너는 앱의 러너 목록에 나타나지 않는다.** 오퍼레이터는 자기가 spawn 한 것만
장부에 갖고, 앱은 그 장부를 읽을 뿐이다 — 서버 presence 에 그 계정이 보이면 얼굴은 초록이
되지만 pid 같은 사실은 없다. 예전 이 자리에는 *"그 러너는 앱에서 '외부에서 실행 중'으로
보인다"* 고 적혀 있었는데 **두 겹으로 틀렸다**: `external` 이라는 상태가 `#482` 에서
없어졌고, 손으로 띄운 러너는 애초에 그렇게 보이지도 않는다.
...
### PAT 는 오퍼레이터가 받아 간다 — 단계 4 까지의 임시 경로

앞 판본은 *"PAT 는 앱이 쥐고, 회전은 사람이 누를 때만"* 이었다 — 앱이 `POST
/accounts/:id/pats` 로 발급받아 OS 키체인에 두고 러너에 넘겼고, 설정에 "PAT 재발급" 버튼이
있었다. **둘 다 사라졌다.** 지금은 배정을 받은 오퍼레이터가 `POST /operator/agents/:id/pat`
로 서버에서 직접 받아 자기 시크릿 디렉터리(0600 파일)에 두고 러너에 넘긴다. 사람이 옮겨
적을 일도, 재발급을 누를 일도 없다. 이 경로 자체가 임시다 — 단계 4 에서 러너는 서버에 직접
붙지 않게 되고("배정이 곧 인가"), 에이전트 PAT 는 없어진다.

옛 PAT 로 돌던 러너는 여전히 다음 호출에서 401 을 받고 **종료 코드 78**로 스스로 물러난다 —
서버가 진실의 원천이라는 성질은 그대로다. 배정이 살아 있으면 오퍼레이터가 백오프 뒤 다시
띄운다.
